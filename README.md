...
Cada pasta é um programa independente, que você executa com `go run ./pipeline/`, e tem um `README.md` com a explicação do padrão. Cada uma tem também um `exemplo_test.go` que confere a saída. Quando o exemplo não depende do relógio, o teste é uma função `Example`, que usa `// Output:` quando a ordem das linhas é fixa e `// Unordered output:` quando só o conjunto é previsível. Quando o exemplo dorme, espera um _ticker_ ou tem prazo, o teste roda dentro de `synctest.Test`, do pacote `testing/synctest`. Lá dentro, na chamada bolha, o relógio é falso e só anda quando todas as gorrotinas estão bloqueadas. Bloqueada, para a bolha, é a gorrotina parada em canal, em `select`, em `sync.WaitGroup.Wait`, em `sync.Cond.Wait` ou em `time.Sleep`. Esperando um mutex, um arquivo, a rede ou uma chamada de sistema ela não conta, e o relógio não anda. Por isso o pacote de captura grava em um arquivo temporário em vez de ler de um `os.Pipe`. Um `time.Sleep` de um segundo termina na hora, e o tempo medido é sempre o mesmo. `synctest.Test` pede um `*testing.T`, que uma `Example` não recebe, então esses testes são funções `Test` e capturam a saída com o pacote [`internal/saida`](./internal/saida/saida.go). A bolha ainda confere uma coisa a mais: se sobrar gorrotina viva quando o teste acaba, ele falha. Para rodar tudo com o detector de corrida, use `go test -race ./...`.
