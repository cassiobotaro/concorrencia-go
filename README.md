...
[Concorrência não é paralelismo](https://go.dev/blog/waza-talk). Concorrência é compor computações independentes, uma forma de estruturar o programa. Paralelismo é executá-las ao mesmo tempo. Um programa concorrente roda em um único processador, e um bem estruturado costuma paralelizar bem quando há mais de um.

O texto parte do princípio de que você conhece gorrotinas, canais, `select` e `sync.WaitGroup`. Se alguma dessas peças for nova, passe antes pelo [Tour of Go](https://go.dev/tour/concurrency/1), pelo [Go by Example](https://gobyexample.com/goroutines) e pelo [Effective Go](https://go.dev/doc/effective_go#concurrency). Para revisar todas, um capítulo por peça, há o livro [Go Concurrency Distilled](https://antonz.org/go-concurrency-distilled/). Para ir mais fundo em `context`, [este repositório](https://github.com/cassiobotaro/contexto).

Os `fmt.Print` dos exemplos existem só para você enxergar a execução e não fazem parte dos padrões.

Cada pasta é um programa independente, que você executa com `go run ./pipeline/`, e tem um `README.md` que explica o padrão e um `exemplo_test.go` que confere a saída. Os testes servem de base para escrever os seus: os que não dependem do relógio são funções `Example`, e os que dormem ou têm prazo rodam dentro de `synctest.Test`, com o relógio falso. Para rodar tudo com o detector de corrida, `go test -race ./...`.

Os mesmos padrões aparecem com outros nomes em livros e artigos, por isso cada README traz uma linha "Também conhecido como". "Produtor" e "consumidor" são papéis, não padrões, e quase todo exemplo tem os dois. Aparecem como nomes alternativos de [Geradores](./geradores/README.md) e [Trabalhador](./trabalhador/README.md) porque nesses dois cada papel aparece sozinho.
