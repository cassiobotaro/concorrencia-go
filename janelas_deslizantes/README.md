...
O truque aqui é o canal `nil`, visto no [fan-in com select](../fan_in/README.md#fan-in-com-uma-goroutine-e-select). Como um `case` cujo canal é `nil` nunca é escolhido, dá para ligar e desligar cada `case` conforme o estado da fila. Quando a fila está vazia, o canal de envio fica `nil` e o `case` de envio é desabilitado, pois não há o que enviar. Quando a entrada é fechada, a variável `entrada` passa a valer `nil` e o `case` de recebimento é desabilitado. Daí em diante só resta esvaziar a fila.

Como só uma _goroutine_ toca a fila, não há disputa entre produtor e consumidor pelo estado. É a técnica da [goroutine dona do estado](../dono_do_estado/README.md). Uma versão anterior deste exemplo usava um canal com buffer compartilhado por duas _goroutines_ e tinha uma corrida sutil que podia travar o programa.
